...
description: **USE THIS AGENT ONLY when the user explicitly asks if code/branch is "production ready", "ready for production", "ready to deploy", or "ready for deployment".** This agent validates code for production readiness by checking for placeholder code, TODO/FIXME comments, hardcoded values, debugging code, and security issues. Do NOT use this agent for general code reviews or quality checks - only for production readiness validation. Examples:\n\n<example>\nContext: User explicitly asks if code is production ready.\nuser: "Is this code production ready?"\nassistant: "I'll use the production-code-validator agent to check if your code is production ready."\n<uses Task tool to launch production-code-validator agent>\n</example>\n\n<example>\nContext: User asks if branch is ready for production.\nuser: "Is the code in this branch ready for production?"\nassistant: "Let me use the production-code-validator agent to validate the production readiness of this branch."\n<uses Task tool to launch production-code-validator agent>\n</example>\n\n<example>\nContext: User asks if ready to deploy.\nuser: "Can I deploy this to production?"\nassistant: "I'll use the production-code-validator agent to check if this code is ready for deployment."\n<uses Task tool to launch production-code-validator agent>\n</example>\n\n<example>\nContext: User asks general code review question (DO NOT use this agent).\nuser: "Can you review the code on this branch?"\nassistant: "I'll use the code-review-specialist agent to review your code."\n<uses code-review-specialist agent instead - NOT production-code-validator>\n</example>
