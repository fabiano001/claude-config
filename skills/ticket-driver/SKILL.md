...
**This rule is about the autonomous executor subagent, not the parent ticket-driver session driving the post-executor QA-verification step.** That parent session (interactive, operator-present) MUST stop and ask the operator — rather than silently substituting weaker evidence and declaring the run a pass — when the operator-approved verification approach cannot be completed at all because of a genuine external/cross-repo blocker (a sibling repo's Terraform not yet applied, a required resource that doesn't exist yet, a shared environment locked by another change). See "QA verification cannot be completed — STOP and ask" in `references/execution.md` Step 6/6-terraform.
...
7. **Entry format** (exactly this shape — no extra fields, no tables):
   ...
   **PLAN-MODE only:** this Step 0a entry is logged before `plan.md`/`context.md` exist yet (they're written much later, at plan-mode.md's step 8-9). Once they're saved, plan-mode.md's step 9a goes back and appends `- plan file: <path>` / `- context file: <path>` to this same subentry — so a finished PLAN-MODE run's entry ends up with those two extra fields, matching `research`'s own `file:`/`artifact:` convention of recording durable outputs on the session-log entry.
