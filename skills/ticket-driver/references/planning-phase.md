...
- **Terraform repos skip this whole section.** If this ticket's repo (`REPO_NAME`) contains `terraform` case-insensitively (e.g. `terraform-stack-trident`), do NOT ask the yes/no question or draft a proposal below — there's no funnel to test with Playwright. Note in the plan output instead: "E2E test: not applicable (terraform repo) — verified via Atlantis (`atlantis plan -p bg-qa` / `atlantis apply -p bg-qa`) during execution; see execution.md's Step 6-terraform." Omit the E2E task from the HIGH LEVEL PLAN and Task Breakdown exactly as the opt-out case below does, and skip straight to the "Jira finalization" ask above (hours to log) if it hasn't already been asked. Continue to the rest of this document as normal — only this section's question/proposal is skipped.

- Otherwise, first ask: **"Do you want to include an E2E test (Playwright CLI) as the final step? (yes/no — default: yes)"**
...
- **Mid-execution updates to "How to verify passing" are allowed but must be documented:** During execution, the executor may discover that the agreed verification approach doesn't work as expected (e.g., the dataLayer is on a child iframe instead of the parent, the expected console log is gated behind a feature flag, the screenshot target re-renders mid-capture). The executor MAY switch to a different verification approach IF AND ONLY IF:
  ...
  2. The new approach successfully validated the same pass/fail intent (i.e., still proved the AC is met) — a "lighter" approach that only proves a NARROWER claim (e.g., "the CI pipeline runs" instead of "the deployed behavior is correct") does NOT satisfy this and is NOT a valid substitution.
  ...
  **This is not the same thing as verification being genuinely blocked** (a sibling repo's infra isn't deployed yet, a resource doesn't exist, a secret hasn't been populated). A blocker means the AC cannot be proven at all right now, by any approach — that is never a valid substitution, no matter how much can still be shown. See `execution.md`'s "QA verification cannot be completed — STOP and ask": the run must stop and ask the operator rather than accept partial proof as a pass.
...
- **ALWAYS include these final steps:**
  ...
  3. Commit + push per feature (see **Feature Grouping** below) — the FIRST feature's commit also pushes the branch and creates the PR **as a draft** (`gh pr create --draft`, which stays draft permanently — this skill never marks it ready for review) — **except a terraform repo, which is immediately taken back out of draft (`gh pr ready`) right after creation; see step 7 below**; each subsequent feature's commit pushes to that same branch/PR, no new PR created
  ...
  6. Run `/codex-review` with the PR URL in autonomous mode again — **final pass** on the now-final diff — **but ONLY if step 5's `/review-pr-comments` run actually produced new commits; skip this pass entirely if it found nothing to fix**, since the diff would be identical to what step 4's pre-clean pass already reviewed (auto-fix + push any remaining real issues when it does run; any late Bugbot comment from this push is reaped by `jira-sprint-manager` Rule C's pre-merge gate — do NOT loop back to review-pr-comments)
  7. **Non-terraform repos:** E2E test using Playwright CLI — drive the funnel manually with snapshot → click/fill → snapshot (do NOT rely on bundled flow scripts; see [e2e.md](e2e.md)). Check `playwright-cli --help` for available commands. Use the URL, test items, and capture criteria collected during planning. **NEVER ask the operator how to proceed with the E2E step** — the plan-review loop already authorized "run E2E + allow up to 5 deploys with `--fix-and-retry`". Do not use `AskUserQuestion` or numbered-option prompts here; do not pause for deploy approval (the plan-review confirmation IS the explicit approval; stage-only safety guards inside `e2e-test-jira-ticket` are the binding check). **Omit this step entirely if the operator opted out of E2E testing during planning.** **Terraform repos run the Atlantis QA workflow instead** (see execution.md's Step 6-terraform): comment `atlantis plan -p bg-qa` on the PR (which the FIRST feature's commit already took out of draft), wait for Atlantis's plan comment, review it for accuracy against this ticket's intended changes and report any discrepancy, then comment `atlantis apply -p bg-qa` once it checks out. (The analogous production `atlantis plan -p bg-prod` / `atlantis apply -p bg-prod` is a reminder for later, out of this run's scope — prod apply always needs the human operator's explicit go-ahead.)
  8. **On QA verification pass: post "QA Pass" Jira comment** via `mcp__atlassian__addCommentToJiraIssue` using the structured format from [qa-pass-comment-template.md](qa-pass-comment-template.md). Title: `# QA Pass (Automated E2E Execution) ✅` (non-terraform) or `# QA Pass (Atlantis Apply — bg-qa) ✅` (terraform, evidence = the Atlantis plan/apply comment URLs). **Skip if E2E failed (`E2ETEST-Report.md` exists), Atlantis QA failed (`ATLANTIS-QA-Report.md` exists), or E2E was opted-out during planning (non-terraform only)** — there is no proof to attach.
  9. **Jira finalization (only on success — E2E passed, or E2E skipped during planning):** assign to Fabiano Desouza (`accountId: 5a6765563c7f1842c3d7b806`), log the hours collected during planning via `mcp__atlassian__addWorklogToJiraIssue`, and post a `Ticket Driver Implementation Completed (<REPO_NAME>): <timestamp>` marker comment (the `Ticket Driver Implementation Completed (<REPO_NAME>)` prefix, repo name included, is what a future run's pre-run Guard step checks for to avoid re-running on this ticket **for this same repo** — the same Guard also checks for a `Ticket Driver Implementation Started (<REPO_NAME>): <timestamp>` comment that this run itself posted right before execution began; a marker for a different repo on the same ticket never blocks this one). **Standard mode also transitions** the ticket to "Ready for QA" — **TODO-MODE does NOT transition it** (the ticket is expected to still be in a TODO-column status the whole time; that mode's whole point is finishing the work ahead of the ticket being picked up). **Skip the entire finalization (transition + assign + log + marker comment) if (a) E2E failed after 5 iterations** (an `E2ETEST-Report.md` was written) **or, for a terraform repo, Atlantis QA failed** (an `ATLANTIS-QA-Report.md` was written), **OR (b, standard mode only) the ticket's current Jira status is not "In Progress"** (re-fetch via `mcp__atlassian__getJiraIssue` immediately before mutating; this prevents re-runs on already-finalized tickets from regressing state — TODO-MODE skips this particular check since "not In Progress" is its expected, normal state, not a re-run signal). On skip, print a clear message naming the reason and continue.
  ...
  **⚠️ MANDATORY CHECKLIST (verify before presenting plan):**
  ...
  - Example format:
    ...
    8. Commit + push per feature (see Feature Grouping) — draft PR created on the first feature's commit
    9. Run /codex-review with PR URL in autonomous mode (pre-clean pass)
    10. Run /review-pr-comments with PR URL in autonomous mode
    11. Run /codex-review with PR URL in autonomous mode (final pass on the final diff — skipped if step 10 made no new commits)
...
- **Commit, Push, and PR (distributed across features — see Feature Grouping below)**: Each feature's tasks, once implemented and passing, get committed as that feature completes — not held until every task in the ticket is done. The FIRST feature's commit is followed by pushing the branch and creating the PR **as a draft** (`gh pr create --draft`) — this PR stays in draft permanently; ticket-driver never marks it ready for review. **Exception: a terraform repo (`REPO_NAME` contains `terraform`) is immediately taken out of draft (`gh pr ready`) right after creation** — see the Atlantis QA task below, which needs the PR presented as ready for review, not draft. Each SUBSEQUENT feature's commit pushes to that same branch — no new PR. After Code Review + Production Validation (above) run against the FULL accumulated diff, any fixes they require land in one final commit + push to the same branch (no new PR either).
...
- **Codex Review — final pass (gated)**: **Only if the preceding Review PR Comments step actually produced new commits** (compare the git HEAD SHA before and after that step) — run `/codex-review` with the PR URL in autonomous mode again, a final correctness pass on the now-final diff; auto-fix + push any remaining real issues. Any late Bugbot comment from this push is reaped by `jira-sprint-manager` Rule C's pre-merge gate; do NOT loop back to review-pr-comments. **If Review PR Comments found nothing to fix (no new commits), skip this pass entirely** — the diff is unchanged since the pre-clean pass already reviewed it, so re-reviewing it would be redundant.
- **E2E Test (Playwright CLI) — non-terraform repos** *(omit this task entirely if the operator opted out of E2E testing during planning, OR if this is a terraform repo — see the Atlantis task below instead)*: Run an E2E test using Playwright CLI. **Drive the funnel manually with `playwright-cli` snapshot → click/fill → snapshot — do NOT rely on any bundled funnel-driving scripts (`*-flow.sh` wrappers); UIs drift faster than scripts get maintained. See [e2e.md](e2e.md) for the loop.** Check `playwright-cli --help` for available commands. Use the **E2E Test Plan** captured during planning (URL, what to test, what to capture). **On failure, run a fix-and-retry loop (max 5 iterations): investigate → fix code → commit/push → re-run the same E2E test.** If still failing after 5 iterations, write `E2ETEST-Report.md` at the project root with status, what's failing, what was tried each iteration, and suggested next steps.
- **Atlantis QA plan/apply — terraform repos only** *(replaces the E2E Test task above; omit this task entirely for non-terraform repos)*: Comment `atlantis plan -p bg-qa` on the PR (already out of draft per the exception above), wait for Atlantis's plan comment, review it for accuracy against this ticket's intended infrastructure changes, and report any discrepancy (unexpected destroy/replace, out-of-scope resource, plan error). If it checks out, comment `atlantis apply -p bg-qa` and confirm the apply actually succeeded. On any discrepancy or apply failure, write `ATLANTIS-QA-Report.md` at the project root (same role as `E2ETEST-Report.md`) and stop — no QA Pass comment, no Jira finalization. See execution.md's Step 6-terraform for the full sequence, including the reminder that the analogous production `atlantis plan -p bg-prod` / `atlantis apply -p bg-prod` is out of this run's scope and prod apply always needs the human operator's explicit authorization.
- **QA Pass Jira comment (only on QA verification pass)**: Post a structured comment to the Jira ticket via `mcp__atlassian__addCommentToJiraIssue` with title **`# QA Pass (Automated E2E Execution) ✅`** (non-terraform) or **`# QA Pass (Atlantis Apply — bg-qa) ✅`** (terraform) and the format defined in [qa-pass-comment-template.md](qa-pass-comment-template.md). The comment must include: run metadata (PR, branch, app version verified live, run date), what was tested (test URL, deploy command, tabs traversed — or, for terraform, the Atlantis plan/apply comment URLs), what was verified (AC-by-AC pass table), evidence (milestone dataLayer JSONs, direct smoke-test responses where applicable, screenshot paths — or the Atlantis plan output for terraform), code-review iterations table (Cursor/Codex findings + fix commit SHAs), local artifacts list, and notes on anything surprising (e.g., funnel-specific quirks). **Skip if (a) E2E failed (`E2ETEST-Report.md` exists at project root), (b) Atlantis QA failed (`ATLANTIS-QA-Report.md` exists), OR (c) operator opted out of E2E during planning (non-terraform only)** — there is no proof to attach.
- **Jira finalization on success**: Assign to **Fabiano Desouza** (`accountId: 5a6765563c7f1842c3d7b806`), log **<HOURS>** hours via `mcp__atlassian__addWorklogToJiraIssue` (where `<HOURS>` is the value the operator provided during planning), and post a `Ticket Driver Implementation Completed (<REPO_NAME>): <timestamp>` marker comment (the `Ticket Driver Implementation Completed (<REPO_NAME>)` prefix, repo name included, is machine-matched by a future run's pre-run Guard check for that same repo — a `Ticket Driver Implementation Started (<REPO_NAME>): <timestamp>` comment was already posted right before execution began, for the same Guard to catch a concurrent run on the same repo even before this one finishes; a different repo on the same ticket is never blocked by either marker). **Standard mode also transitions** the ticket to **"Ready for QA"**; **TODO-MODE does not** — it leaves the ticket's status untouched so it's ready to transition the moment it's actually picked up later. **Skip the entire finalization (transition + assign + log + marker comment) if (a) E2E failed after 5 iterations** (an `E2ETEST-Report.md` was written) **or, for a terraform repo, Atlantis QA failed** (an `ATLANTIS-QA-Report.md` was written), **OR (b, standard mode only) the ticket's current Jira status is not "In Progress"** — re-fetch via `mcp__atlassian__getJiraIssue` immediately before mutating; if it's already past finalization (e.g., re-running on a ticket already at "Ready for QA"), print a clear skip message and continue. (TODO-MODE never applies this particular check — being outside "In Progress" is its normal, expected state.) Run this even when E2E was skipped during planning.

### 5a) Feature Grouping (mini-features for incremental commits + an early draft PR)

Group the Task Breakdown's tasks into **mini-features** — each a self-contained, independently-committable unit of work (its implementation tasks plus the tests that cover them). Use your own discretion on how to split:

- **Small tickets** (a handful of tasks, one clear unit of work) — a single feature covering everything is fine: one feature, one commit. Do not force a split just to produce more than one commit.
- **Mid-size or large tickets** — split into 2, 3, or more features along natural seams: by layer (backend endpoint vs. frontend UI vs. shared types), by acceptance-criteria grouping, or by independently-testable behavior. Each feature should leave the branch in a working, lint-clean, test-passing state on its own — never a half-finished slice.

**Order matters:** put the most foundational/base piece first (e.g., shared types, a new endpoint's happy path) — the FIRST feature's commit is also when the branch gets pushed and the PR is created (as a draft — see the final-steps list above), so it should be something coherent enough to open a PR around. Later features add commits on top of that same open PR, in dependency order (each one should build on what's already landed, not require reordering).

**Always fold the semantic-versioning tasks (if `dynamic-app/` is touched) into feature 1's own scope** — they're a project-wide bump, not tied to any one later feature, so they belong in the first commit rather than deferred.

Present as a numbered list, each entry naming which Task Breakdown item(s) it covers:

```
## FEATURE GROUPING
1. **<Feature name>** — tasks 1–3 (tests + implementation for <what>)
2. **<Feature name>** — tasks 4–5 (tests + implementation for <what>)
…
```
...
- **Feature Grouping** (mini-features mapped to Task Breakdown items — determines commit-and-draft-PR order during execution; a small ticket may have just one)
