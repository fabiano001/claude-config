...
Run bookkeeping (the pre-run "Guard" check, Step 0 start timestamp, and Step 0a sessions.md logging — all from SKILL.md) STILL applies in PLAN-MODE. In particular, if the ticket already has a `Ticket Driver Implementation Started (<REPO_NAME>): <timestamp>` OR `Ticket Driver Implementation Completed (<REPO_NAME>): <timestamp>` comment for THIS repo from a prior run, the Guard step stops this run before it plans anything — there's no point re-planning a ticket whose implementation is already underway or fully done for this repo. (A marker for a *different* repo on the same ticket doesn't stop planning for this one.) (PLAN-MODE itself never posts either marker — it never executes, so there's no "started"/"completed" implementation event to mark.)
...
1. **Collect inputs** — Same as standard mode: fetch from Jira, accept manual inputs/overrides, and check for ARTIFACT mode (see [planning-phase.md](planning-phase.md) → "Jira integration" and "ARTIFACT mode"). ARTIFACT mode composes with PLAN-MODE exactly as with Standard mode — if detected, Story/Description/Technical Details/Testing Methodology/Acceptance Criteria/Deployment Notes/Rollback Steps come entirely from the artifact, and that content is what gets written into `context.md` below (the `## Technical Documentation` section holds the artifact's "Technical Details" content in that case, same placeholder, different source).
...
8. **Save `plan.md`** — Distill the finalized plan into a checklist and write to:
   ...
   - [ ] Run `/codex-review` with the PR URL in autonomous mode — **pre-clean pass** (runs first, synchronously within this session via an independent Opus 4.8 subagent review — no waiting on external bots; pre-empts Bugbot findings so review-pr-comments runs fewer iterations)
   - [ ] Wait 20 minutes for review bots to post comments: `sleep 1200`
   - [ ] Run `/review-pr-comments` with the PR URL in autonomous mode to auto-fix reviewer feedback (terminal convergence loop)
   - [ ] Run `/codex-review` with the PR URL in autonomous mode again — **final pass** on the now-final diff — **GATED: only if the review-pr-comments step above produced new commits (compare git HEAD before/after); skip entirely if it found nothing to fix, since the diff would be unchanged from the pre-clean pass** (when it does run: auto-fix + push any remaining real issues; any late Bugbot comment from this push is reaped by jira-sprint-manager Rule C's pre-merge gate — do NOT loop back to review-pr-comments)
...
9a. **Record the plan/context files on the sessions.md entry** — Step 0a (in SKILL.md) already logged a `## ticket-driver (plan mode)` subentry for this run, BEFORE `plan.md`/`context.md` existed, so it has no file paths on it yet. Now that both are saved, go back and add them:
    1. Read `~/.claude/memory/sessions.md` with the Read tool.
    2. Find the `# <TICKET_NAME>` header (same matching rule as Step 0a) and, under it, the `## ticket-driver (plan mode)` subentry whose `session id:` matches this run's own session UUID (already known from Step 0a — no need to re-derive it).
    3. Append two fields to that exact subentry, immediately after its existing `- date:` line:
       ```markdown
       - plan file: ~/RalphLoops/SprintLoop/Sprints/<SPRINT_NAME>/<TICKET_NAME>/plan.md
       - context file: ~/RalphLoops/SprintLoop/Sprints/<SPRINT_NAME>/<TICKET_NAME>/context.md
       ```
       Use the full, `~`-expanded absolute path for both, not a bare filename — same convention `research`'s own `file:` field already uses. Do not touch any other line in the subentry or any other entry in the file.
    4. Write the updated file back with the Write tool — **never** shell redirection (`>`, `>>`) or `echo`.
    5. If the subentry can't be found (e.g. Step 0a's logging failed earlier and printed a warning instead of writing), skip this sub-step silently — there's nothing to append fields to, and Step 0a's own failure handling already surfaced that.

    If this step errors for any other reason, print one warning line and continue to step 10 — it's bookkeeping on top of an already-successful plan, not a gate on PLAN-MODE's own completion.
