...
8. **Save `plan.md`** — Distill the finalized plan into a checklist and write to:
   ...
   - [ ] Run `/codex-review` with the PR URL in autonomous mode again — **final pass** on the now-final diff (auto-fix + push any remaining real issues; any late Bugbot comment from this push is reaped by jira-sprint-manager Rule C's pre-merge gate — do NOT loop back to review-pr-comments)
