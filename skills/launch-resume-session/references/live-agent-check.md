...
### Step A — classify `REPO_PATH` as dedicated or shared

Compute the **identifying token** first, per which heading scheme resolved this session (SKILL.md Step 1):
- Ticket-key lookup (`CREATION`/`DRIVER`/`SPIKE`) → token = `TICKET_KEY` itself (e.g. `TRIDENT-971`).
- Research-description lookup (`RESEARCH`) → token = `TOPIC_SLUG`, i.e. `SHORT_DESCRIPTION` lowercased with spaces/punctuation replaced by hyphens (the exact slugging `launch-research-agent` does for its own worktree path — e.g. `Mobile Finance CTA Flow` → `mobile-finance-cta-flow`).

Check whether `REPO_DIR`'s basename **contains that token as a substring** (e.g. does `webapp-react-trident-TRIDENT-971` contain `TRIDENT-971`? Yes. Does `bt-mobile-app-research-mobile-finance-cta-flow` contain `mobile-finance-cta-flow`? Yes.). This is the actual dividing line found in real data — not "does it look like a worktree path":

- **Contains the token → DEDICATED.** Nothing else has a legitimate reason to run in a directory named after this exact ticket or this exact research topic. Proceed to Step B (cwd-matching; safe here).
- **Does NOT contain the token → SHARED.** Covers the bare main-checkout case (`webapp-react-trident`), the generic-scratch-worktree case (`webapp-react-trident-worktree-research`, reused across unrelated tickets in observed real data), and (for research) any research worktree that predates this skill's slugging convention or was recorded against a shared checkout. Skip Step B — go directly to Step C.

### Step B — DEDICATED: check liveness by `cwd`
...
- **No `sessionId` match** → **do not conclude NOT LIVE and proceed automatically.** The directory is shared, so the absence of a `sessionId` match is not strong enough evidence that no derived-id copy is running there. Instead: report that `REPO_DIR` (`<value>`) is a shared directory, not dedicated to this ticket/topic, liveness for this exact session cannot be reliably confirmed automatically, and hand the operator a ready-made command to run themselves after checking `claude agents` on their own:
  ```
  <identifier> (<TYPE>): repo/dir "<REPO_DIR>" is shared across multiple tickets/topics/sessions, not dedicated to this one — I can't safely confirm whether session <SESSION_ID> is already running there in the background. Please check `claude agents` yourself, then if it's clear, run:
...
- Do not classify by "does the path look like a worktree" (contains a hyphen, contains "research", etc.) — the actual, verified dividing line is whether the identifying token (ticket key, or research topic slug) itself appears in the path. Guessing from shape alone would have still misclassified the `webapp-react-trident-worktree-research` shared case.
